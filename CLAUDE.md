...
once bound; `ServeHandle::shutdown()` stops and joins both of a site's threads — the request loop and
the rebuild watcher — and `Drop` does the same. It is the non-blocking counterpart to `run_serve`,
and **both build from the same `setup()`** — that shared
